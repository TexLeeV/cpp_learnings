...
{
public:
    explicit NonNoexceptMove(const std::string& name) : name_(name)
    {
    ...
    NonNoexceptMove(NonNoexceptMove&& other) : name_(std::move(other.name_))
    {
    ...
    NonNoexceptMove(const NonNoexceptMove& other) : name_(other.name_)
    {
...
{
public:
    explicit NoexceptMove(const std::string& name) : name_(name)
    {
    ...
    NoexceptMove(NoexceptMove&& other) noexcept : name_(std::move(other.name_))
    {
    ...
    NoexceptMove(const NoexceptMove& other) : name_(other.name_)
    {
...
template <typename T> class ConditionalNoexceptWrapper
{
public:
    explicit ConditionalNoexceptWrapper(T value) : value_(std::move(value))
    {
...
TEST_F(NoexceptTest, ConditionalNoexcept_TypeDependent)
{
    ...
    static_assert(!noexcept(NonNoexceptWrapper(std::declval<NonNoexceptWrapper&&>())),
                  "NonNoexceptWrapper move should not be noexcept");
    ...
    static_assert(noexcept(NoexceptWrapper(std::declval<NoexceptWrapper&&>())),
                  "NoexceptWrapper move should be noexcept");
}
...
{
public:
    explicit ExpensiveCopy(int id) : id_(id), data_(std::make_unique<Tracked>("Data" + std::to_string(id)))
    {
    ...
    ExpensiveCopy(const ExpensiveCopy& other)
        : id_(other.id_), data_(std::make_unique<Tracked>("Data" + std::to_string(id_)))
    {
        EventLog::instance().record("ExpensiveCopy::copy_ctor - expensive!");
    }

    // Move is cheap and noexcept
    ExpensiveCopy(ExpensiveCopy&& other) noexcept : id_(other.id_), data_(std::move(other.data_))
    {
...
TEST_F(NoexceptTest, ConditionalNoexcept_MemberFunctions)
{
    ...
    static_assert(std::is_nothrow_move_constructible_v<int>, "int should be nothrow move constructible");
...
public:
    ...
    Wrapper(Wrapper&& other) noexcept(std::is_nothrow_move_constructible_v<T>) : value_(std::move(other.value_))
    {
...
TEST_F(NoexceptTest, Templates_NoexceptPropagation)
{
    ...
    static_assert(std::is_nothrow_move_constructible_v<IntWrapper>, "Wrapper<int> move should be noexcept");
