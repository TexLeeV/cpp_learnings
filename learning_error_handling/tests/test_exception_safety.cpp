...
public:
    ...
    explicit CopyAndSwapAssignment(const std::string& name) : data_(std::make_unique<Tracked>(name))
    {
...
public:
    PartiallyConstructedObject(bool throw_after_first)
        : first_(std::make_unique<Tracked>("First")), second_(nullptr), third_(nullptr)
    {
...
public:
    ...
    explicit AssignmentSafety(const std::string& name) : data_(std::make_unique<Tracked>(name))
    {
