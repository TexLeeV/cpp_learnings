...
// ============================================================================
// Monadic Operations - Chaining Optional Operations
// ============================================================================

std::optional<int> safe_divide(int numerator, int denominator)
{
    EventLog::instance().record("safe_divide: called");
    
    if (denominator == 0)
    {
        EventLog::instance().record("safe_divide: division by zero");
        return std::nullopt;
    }
    
    EventLog::instance().record("safe_divide: success");
    return numerator / denominator;
}

std::optional<int> parse_and_divide(const std::string& num_str, const std::string& denom_str)
{
    EventLog::instance().record("parse_and_divide: start");
    
    auto numerator = parse_int(num_str);
    if (!numerator)
    {
        EventLog::instance().record("parse_and_divide: numerator parse failed");
        return std::nullopt;
    }
    
    auto denominator = parse_int(denom_str);
    if (!denominator)
    {
        EventLog::instance().record("parse_and_divide: denominator parse failed");
        return std::nullopt;
    }
    
    // TODO: Call safe_divide and return the result
    return safe_divide(*numerator, *denominator);
}

// Q: How many explicit nullopt checks are required in parse_and_divide?
// A:
// R:

// Q: How would this compare to using exceptions for error handling?
// A:
// R:

TEST_F(OptionalResultTypesTest, Optional_Chaining)
{
    // Moderate: Chaining optional operations requires explicit checks
    
    auto result1 = parse_and_divide("10", "2");
    EXPECT_TRUE(result1.has_value());
    EXPECT_EQ(*result1, 5);
    
    auto result2 = parse_and_divide("10", "0");
    EXPECT_FALSE(result2.has_value());
    EXPECT_EQ(EventLog::instance().count_events("safe_divide: division by zero"), 1);
    
    auto result3 = parse_and_divide("abc", "2");
    EXPECT_FALSE(result3.has_value());
    EXPECT_EQ(EventLog::instance().count_events("parse_and_divide: numerator parse failed"), 1);
    
    auto result4 = parse_and_divide("10", "xyz");
    EXPECT_FALSE(result4.has_value());
    EXPECT_EQ(EventLog::instance().count_events("parse_and_divide: denominator parse failed"), 1);
}
...
public:
    DataStore()
    {
        EventLog::instance().record("DataStore::ctor");
        data_["key1"] = "value1";
        data_["key2"] = "value2";
    }
    ...
    {
        EventLog::instance().record("DataStore::get('" + key + "')");
        
        auto it = data_.find(key);
        if (it == data_.end())
        {
            EventLog::instance().record("DataStore::get - not found");
            return std::nullopt;
        }
        
        EventLog::instance().record("DataStore::get - found");
        return it->second;
    }
...
// Q: Why does get() return std::optional<std::string> instead of std::optional<std::string&>?
// A:
// R:

// Q: What lifetime issue would arise with std::optional<std::string&>?
// A:
// R:

TEST_F(OptionalResultTypesTest, Optional_ValueSemantics)
{
    // Moderate: std::optional typically uses value semantics for safety
    ...
    EXPECT_EQ(EventLog::instance().count_events("DataStore::get - found"), 1);
    EXPECT_EQ(EventLog::instance().count_events("DataStore::get - not found"), 1);
}

// ============================================================================
// Combining Optional with RAII
// ============================================================================

std::optional<std::unique_ptr<Tracked>> create_tracked_optional(bool should_create)
{
    EventLog::instance().record("create_tracked_optional: called");
    
    if (!should_create)
    {
        EventLog::instance().record("create_tracked_optional: returning nullopt");
        return std::nullopt;
    }
    
    EventLog::instance().record("create_tracked_optional: creating Tracked");
    return std::make_unique<Tracked>("OptionalPtr");
}

// Q: How many levels of "optional-ness" exist in optional<unique_ptr<T>>?
// A:
// R:

// Q: What is the difference between nullopt and unique_ptr holding nullptr?
// A:
// R:

TEST_F(OptionalResultTypesTest, Optional_WithSmartPointers)
{
    // Hard: Combining optional with smart pointers for double-optional semantics
    
    {
        auto result1 = create_tracked_optional(true);
        EXPECT_TRUE(result1.has_value());
        EXPECT_TRUE(result1->get() != nullptr);
        
        EXPECT_EQ(EventLog::instance().count_events("Tracked(OptionalPtr)::ctor"), 1);
        
        // result1 goes out of scope
    }
    
    // Verify Tracked was destroyed
    EXPECT_EQ(EventLog::instance().count_events("Tracked(OptionalPtr)::dtor"), 1);
    
    EventLog::instance().clear();
    
    {
        auto result2 = create_tracked_optional(false);
        EXPECT_FALSE(result2.has_value());
        
        // No Tracked object created
        EXPECT_EQ(EventLog::instance().count_events("Tracked"), 0);
    }
}
...
TEST_F(OptionalResultTypesTest, Performance_OptionalVsExceptions)
{
    // Hard: Understanding performance trade-offs
    
    // std::optional:
    // - Success path: Zero overhead (just return value)
    // - Error path: Zero overhead (just return nullopt)
    // - Explicit checking required at each level
    
    // Exceptions:
    // - Success path: Zero overhead (no throw)
    // - Error path: Expensive (stack unwinding, RTTI lookup)
    ...
    // Q: For a database connection that fails 0.1% of the time, which approach is more efficient?
    // A:
    // R:
}
