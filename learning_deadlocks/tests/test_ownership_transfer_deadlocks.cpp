...
{
public:
    explicit Mailbox(const std::string& name) : name_(name)
    {
    }
...
TEST_F(OwnershipTransferDeadlocksTest, DISABLED_Scenario1_LockInversionDuringTransfer_Fixed)
{
    ...
    std::thread t1([&]() { send_fixed(mb1, std::make_shared<Tracked>("Msg1"), mb2); });

    std::thread t2([&]() { send_fixed(mb2, std::make_shared<Tracked>("Msg2"), mb3); });

    std::thread t3([&]() { send_fixed(mb3, std::make_shared<Tracked>("Msg3"), mb1); });
...
{
public:
    explicit EventHandler(const std::string& name) : data_(std::make_shared<Tracked>(name))
    {
    }
...
TEST_F(OwnershipTransferDeadlocksTest, DISABLED_Scenario2_CallbackWithLockHeld_Broken)
{
    ...
    h1.set_callback([&](std::shared_ptr<Tracked> event) { h2.trigger_deadlock(event); });

    // H2's callback triggers H1
    h2.set_callback([&](std::shared_ptr<Tracked> event) { h1.trigger_deadlock(event); });
...
TEST_F(OwnershipTransferDeadlocksTest, Scenario2_CallbackWithLockHeld_Fixed)
{
    ...
    h1.set_callback([&](std::shared_ptr<Tracked> event) { trigger_fixed(h2, event); });

    h2.set_callback([&](std::shared_ptr<Tracked> event) { trigger_fixed(h1, event); });

    std::thread t1([&]() { trigger_fixed(h1, std::make_shared<Tracked>("Event1")); });

    std::thread t2([&]() { trigger_fixed(h2, std::make_shared<Tracked>("Event2")); });

    std::thread t3([&]() { trigger_fixed(h1, std::make_shared<Tracked>("Event3")); });
...
{
public:
    explicit SharedState(const std::string& name) : data_(std::make_shared<Tracked>(name))
    {
    }
