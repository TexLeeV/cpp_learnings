...
{
public:
    explicit FileHandle(const std::string& filename) : file_(nullptr), filename_(filename)
    {
...
{
public:
    explicit MovableFileHandle(const std::string& filename) : file_(nullptr), filename_(filename)
    {
        file_ = std::fopen(filename.c_str(), "w");
    ...
    MovableFileHandle(MovableFileHandle&& other) noexcept : file_(other.file_), filename_(std::move(other.filename_))
    {
        other.file_ = nullptr;
...
{
public:
    MultiResourceManager(const std::string& file1, const std::string& file2) : handle1_(file1), handle2_(file2)
    {
