...
public:
    ...
    : ptr_(ptr)
    , ref_count_(ptr ? new std::atomic<long>(1) : nullptr)
    {
        EventLog::instance().record("SharedPtr::ctor use_count=" + 
    ...
    , ref_count_(other.ref_count_)
    {
        ...
        EventLog::instance().record("SharedPtr::copy_ctor use_count=" + 
                                   std::to_string(use_count()));
    }
    ...
    SharedPtr& operator=(const SharedPtr& other)
    {
        if (this != &other)
        {
            ...
            EventLog::instance().record("SharedPtr::copy_assign use_count=" + 
                                       std::to_string(use_count()));
        }
        return *this;
    }
...
private:
    void release()
    {
        if (ref_count_)
        {
            ...
            EventLog::instance().record("SharedPtr::release use_count=" + 
                                       std::to_string(count));
