...
public:
    ...
    T* allocate(std::size_t n)
    {
        ...
        oss << "LoggingAllocator::allocate(" << n << " objects, " 
            << (n * sizeof(T)) << " bytes)";
    ...
    {
        std::ostringstream oss;
        oss << "LoggingAllocator::deallocate(" << n << " objects, " 
            << (n * sizeof(T)) << " bytes)";
        EventLog::instance().record(oss.str());
...
public:
    ...
    CountingAllocator()
    : allocation_count_(std::make_shared<size_t>(0))
    , deallocation_count_(std::make_shared<size_t>(0))
    {
    }
