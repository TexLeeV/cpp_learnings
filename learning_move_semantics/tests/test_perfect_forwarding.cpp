...
template <typename T> void sink(T&& param)
{
    EventLog::instance().record("sink called");
}

template <typename T> void forward_to_sink(T&& param)
{
...
template <typename T> void imperfect_forward(T&& param)
{
...
template <typename T, typename... Args> std::unique_ptr<T> make_unique_impl(Args&&... args)
{
...
TEST_F(PerfectForwardingTest, VariadicForwarding)
{
    ...
    // Q: How does Args&&... with std::forward<Args>(args)... preserve the value category of each argument
    // independently? A: R:
...
template <typename T> void universal_ref(T&& param)
{
...
TEST_F(PerfectForwardingTest, UniversalReferenceVsRvalueReference)
{
    ...
    auto rvalue_only = [](MoveTracked&& param) { EventLog::instance().record("rvalue_only called"); };
...
template <typename T> void type_deduction_forward(T&& param)
{
...
TEST_F(PerfectForwardingTest, TypeDeductionRules)
{
    ...
    // Q: When type_deduction_forward(std::move(obj)) is called, what is T deduced as and what is T&& after reference
    // collapsing? A: R:
...
public:
    ...
    template <typename U> void forward_set(U&& value)
    {
...
template <typename... Args> void forward_multiple(Args&&... args)
{
...
template <typename T> T&& forward_reference_collapsing(T&& param)
{
...
template <typename T> void deduce_and_forward(T&& param)
{
