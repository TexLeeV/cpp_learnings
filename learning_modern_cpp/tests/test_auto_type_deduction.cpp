...
TEST_F(AutoTypeDeductionTest, BasicAutoDeduction)
{
    ...
    static_assert(std::is_same<decltype(ptr), std::shared_ptr<Tracked>>::value, "ptr should be shared_ptr<Tracked>");
...
TEST_F(AutoTypeDeductionTest, AutoReturnTypeDeduction)
{
    ...
    static_assert(std::is_same<decltype(ptr), std::shared_ptr<Tracked>>::value, "Should be shared_ptr<Tracked>");
