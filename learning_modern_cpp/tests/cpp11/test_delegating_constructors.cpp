...
public:
    ...
    {
        EventLog::instance().record("Resource::ctor(name=" + name_ + ", value=" + std::to_string(value_) + ")");
    }
...
public:
    explicit TrackedContainer(const std::string& name, int capacity)
        : data_(std::make_shared<Tracked>(name)), capacity_(capacity)
    {
        EventLog::instance().record("TrackedContainer::main_ctor");
    }

    explicit TrackedContainer(const std::string& name) : TrackedContainer(name, 10)
    {
...
{
public:
    explicit Base(const std::string& name) : name_(name)
    {
        EventLog::instance().record("Base::ctor(name=" + name_ + ")");
    }

    Base(const std::string& name, int value) : name_(name), value_(value)
    {
        EventLog::instance().record("Base::ctor(name=" + name_ + ", value=" + std::to_string(value_) + ")");
    }
