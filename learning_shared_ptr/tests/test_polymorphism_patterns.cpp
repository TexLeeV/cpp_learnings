...
{
public:
    explicit Base(const std::string& name) : tracked_(name)
    {
    }
...
{
public:
    explicit Derived(const std::string& name) : Base(name), derived_tracked_(name + "_derived")
    {
    }
...
TEST_F(PolymorphismPatternsTest, DynamicPointerCastBasic)
{
    ...
    // Q: Why do both `base_ptr` and `derived_ptr` report `use_count() == 2`? What does this reveal about the control
    // block? A: R:
...
TEST_F(PolymorphismPatternsTest, DynamicPointerCastFailure)
{
    ...
    long after_cast_base_count = base_ptr.use_count();
    // Q: When `dynamic_pointer_cast` fails, why does `after_cast_base_count` remain at 1? What does this reveal about
    // failed cast behavior? A: R:
...
TEST_F(PolymorphismPatternsTest, ConstPointerCast)
{
    ...
    // Q: After `const_pointer_cast`, both pointers report `use_count() == 2`. What does this reveal about
    // const-casting's impact on the underlying object? A: R:
...
{
public:
    explicit WidgetImpl(const std::string& name) : tracked_(name)
    {
    }
...
{
public:
    explicit Widget(const std::string& name) : pimpl_(std::make_shared<WidgetImpl>(name))
    {
    }
...
TEST_F(PolymorphismPatternsTest, PimplIdiomBasic)
{
    ...
    // Q: After copying `w1` to `w2`, what does `use_count_copied == 2` reveal about the implementation object's
    // lifetime? A: R:
...
{
public:
    explicit ConcreteA(const std::string& name) : tracked_(name)
    {
    }
...
{
public:
    explicit ConcreteB(const std::string& name) : tracked_(name)
    {
    }
...
TEST_F(PolymorphismPatternsTest, DowncastingInHierarchy)
{
    ...
    // Q: Why does `interface_count` equal 2 after two casts, one of which failed? What contributes to the reference
    // count? A: R:
...
{
public:
    explicit MultiDerived(const std::string& name) : Base(name), multi_tracked_(name + "_multi")
    {
    }
...
TEST_F(PolymorphismPatternsTest, MultiLevelHierarchyCasting)
{
    ...
    // Q: Why does casting to `Derived` fail while casting to `MultiDerived` succeeds? What does this reveal about the
    // actual object type? A: R:
...
TEST_F(PolymorphismPatternsTest, AliasingWithPolymorphicTypes)
{
    ...
    {
        Base base_member;
        explicit Container(const std::string& name) : base_member(name)
        {
        }
    };
    ...
    // Q: The aliasing constructor points `alias` to `base_member` but shares ownership with `container`. What would
    // happen if `container` were destroyed while `alias` still exists? A: R:
