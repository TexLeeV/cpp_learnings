...
TEST_F(AntiPatternsTest, SharedPtrAsGlobal)
{
    ...
    // Q: If this were a global shared_ptr, what observable problem would arise when multiple translation units access
    // it during static initialization? A: R:
...
{
    auto process = [](std::shared_ptr<Tracked> item) { auto copy = *item; };
    ...
    // Q: What observable overhead does passing `shared_ptr` by value introduce compared to passing by `const
    // shared_ptr&`? A: R:
...
TEST_F(AntiPatternsTest, HoldingSharedPtrToThis)
{
    ...
    {
    public:
        explicit BadSelfReference(const std::string& name) : tracked_(name)
        {
        }
...
TEST_F(AntiPatternsTest, UsingSharedPtrForUniqueOwnership)
{
    ...
    // Q: If `use_count` is always 1 throughout the object's lifetime, what runtime overhead does shared_ptr impose
    // compared to unique_ptr? A: R:
...
TEST_F(AntiPatternsTest, StoringWeakPtrWhenSharedPtrNeeded)
{
    ...
    // Q: What design decision would lead you to store a weak_ptr instead of a shared_ptr? What ownership semantics does
    // this express? A: R:
...
TEST_F(AntiPatternsTest, HoldingSharedPtrInUnstableContainer)
{
    ...
    // Q: After the scope exits, why does `dtor_count` equal 0? What ownership decision does storing `shared_ptr` in the
    // cache express? A: R:
