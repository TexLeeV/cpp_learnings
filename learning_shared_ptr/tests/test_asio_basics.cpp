...
TEST_F(AsioBasicsTest, Lesson1_BasicPostAndRun)
{
    ...
    asio::post(io, [&callback_executed]() { callback_executed = true; });
...
TEST_F(AsioBasicsTest, Lesson4_CaptureByValueVsReference)
{
    ...
    // Capture by value
    asio::post(io, [value, &result_by_value]() { result_by_value = value; });

    // Capture by reference
    asio::post(io, [&value, &result_by_reference]() { result_by_reference = value; });
...
TEST_F(AsioBasicsTest, Lesson5_SharedPtrWithPost)
{
...

    {
        ...
        asio::post(io, [ptr, &use_count_in_callback]() { use_count_in_callback = ptr.use_count(); });
...
{
public:
    explicit SimpleClass(const std::string& name) : tracked_(name)
    {
    }
...
TEST_F(AsioBasicsTest, Lesson12_BindVsLambda)
{
    ...
    asio::post(io, [obj]() { obj->method(20); });
...
{
public:
    explicit AsyncWorker(const std::string& name) : tracked_(name), completed_(false)
    {
    }
