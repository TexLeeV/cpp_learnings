...
TEST_F(AliasingWeakTest, AliasingBasic)
{
    ...
    // R: For junior SWEs: Think of it like two keys to the same house (control block), even though one key is labeled "front door" (Container*) and the other "bedroom" (Tracked*).
    ...
    // R: For junior SWEs: The control block remembers "delete the Container" regardless of which shared_ptr goes away last.
    ...
    // R: For junior SWEs: alias.get() returns the "aliased" pointer (the member), not the owned pointer (the Container).
...
TEST_F(AliasingWeakTest, AliasingKeepsOwnerAlive)
{
    ...
    
    {
        ...
        // R: For junior SWEs: alias is still holding the Container alive through shared ownership, even though it points to the member.
        ...
        // R: For junior SWEs: One shared_ptr left = use_count of 1.
    }
    ...
    // R: For junior SWEs: alias outlives the inner scope, so the Container outlives it too.
    ...
    // R: For junior SWEs: Last shared_ptr dies → object dies → control block dies (if no weak_ptrs).
...
TEST_F(AliasingWeakTest, WeakPtrBasic)
{
    ...
    // R: For junior SWEs: No control block = expired, like an empty mailbox address.
    
    {
        ...
        // R: For junior SWEs: weak_ptr observes but doesn't own—use_count stays at 1.
        ...
        // R: For junior SWEs: weak.use_count() tells you how many shared_ptrs are alive, not how many weak_ptrs.
    }
    ...
    // R: For junior SWEs: Object deleted → control block says "expired" → weak.expired() returns true.
    ...
    // R: For junior SWEs: weak_ptr keeps the control block alive (for bookkeeping) but not the object itself.
...
TEST_F(AliasingWeakTest, WeakPtrLock)
{
    ...
    // R: For junior SWEs: One owner = use_count of 1.
    ...
    // R: For junior SWEs: lock() promotes the weak_ptr to a shared_ptr, adding one more owner.
    ...
    // R: For junior SWEs: As long as any shared_ptr is alive, the object stays alive.
    ...
    // R: For junior SWEs: lock() creates a copy of the shared_ptr, just like assignment does.
...
TEST_F(AliasingWeakTest, WeakPtrExpiredLock)
{
    ...
    // R: For junior SWEs: Object gone = expired.
    ...
    // R: For junior SWEs: Can't lock what's already gone—lock() returns nullptr for expired weak_ptrs.
    ...
    // R: For junior SWEs: Always safe to call lock()—it returns nullptr if expired, no crashes.
    ...
    // R: For junior SWEs: Dereferencing nullptr = crash. Always check: if (locked_ptr) { use it }.
...
TEST_F(AliasingWeakTest, AliasingWithWeakPtr)
{
    ...
    
    {
        ...
        // R: For junior SWEs: Two shared_ptrs = use_count of 2.
        ...
        // R: For junior SWEs: weak inherits alias's stored pointer (member), but shares the control block (Container ownership).
        ...
        // R: For junior SWEs: "The Container is deleted" is the clearest—destructor runs, memory freed.
        ...
        // R: For junior SWEs: weak_ptr doesn't keep objects alive, only the control block metadata.
    }
    ...
    // R: For junior SWEs: expired = object deleted, can't lock() it anymore.
...
TEST_F(AliasingWeakTest, WeakPtrUseCount)
{
    ...
    // R: For junior SWEs: weak.use_count() tells you "how many shared_ptrs exist," not "how many weak_ptrs."
    ...
    // R: For junior SWEs: They both read the same number from the control block.
    ...
    // R: For junior SWEs: More weak_ptrs don't change use_count—only shared_ptrs do.
...
TEST_F(AliasingWeakTest, MultipleWeakPtrs)
{
    ...
    // R: For junior SWEs: Repetition helps: weak_ptr = observer, not owner.
    ...
    // R: For junior SWEs: Control block has two counters—use_count for shared_ptrs, weak_count for weak_ptrs.
    ...
    // R: For junior SWEs: Object dies when use_count = 0, but control block survives until weak_count = 0 too.
...
TEST_F(AliasingWeakTest, WeakPtrCopy)
{
    ...
    // R: For junior SWEs: Two weak_ptrs = weak_count of 2.
    ...
    // R: For junior SWEs: shared + locked1 + locked2 = 3 owners.
    ...
    // R: For junior SWEs: All paths lead to the same object—same control block, same pointer.
...
TEST_F(AliasingWeakTest, WeakPtrReset)
{
    ...
    // R: For junior SWEs: One weak_ptr = weak_count of 1.
    ...
    // R: For junior SWEs: Resetting a weak_ptr doesn't touch the shared_ptrs or the object—it just disconnects the observer.
    ...
    // R: For junior SWEs: weak gave up observing, so weak_count = 0.
    ...
    // R: For junior SWEs: Both are empty and expired—no difference in behavior.
