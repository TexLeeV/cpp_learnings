...
TEST_F(ScopeLifetimePatternsTest, ControlBlockCorruption_DoubleControlBlock)
{
    ...
    // Q: At line 381, both p1 and p2 exist and point to the same object. Each has its own control block. When you call
    // p1.use_count(), what memory does it need to access?
    // A:
    // R:

    // Q: Given that p2's control block also points to the same Tracked object, and both control blocks might be trying
    // to manage the same memory, what kind of race condition or memory corruption could occur?
    // A:
    // R:
    ...
    // Q: What information would you get from running the test under a debugger (gdb) and interrupting it during the
    // hang? What specific commands would show you the call stack and what each thread is doing?
    // A:
    // A:
    // A:
    // A:
    ...
    // Q: What would AddressSanitizer (ASan) report if you compiled and ran this test with -fsanitize=address? Would it
    // catch the double control block issue before the hang occurs?
    // A:
    // R:

    // Q: If you added print statements (or EventLog records) immediately before and after line 350
    // (std::shared_ptr<Tracked> p2(raw)), and before line 393 (p1.use_count()), which print statements would you expect
    // to see before the hang?
    // A:
    // R:

    // Q: What would happen if you ran this test with Valgrind? What specific errors would it report about the memory
    // operations?
    // A:
    // R:

    // Q: Could you isolate the problem by creating a minimal test that just does: Tracked* raw = new Tracked("X");
    // shared_ptr<Tracked> p1(raw); shared_ptr<Tracked> p2(raw); long c = p1.use_count(); without any test framework
    // overhead?
    // A:
    // R:
