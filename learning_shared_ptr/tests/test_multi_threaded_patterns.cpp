...
{
public:
    explicit AsyncWorker(const std::string& name) : tracked_(name)
    {
    }
...
{
public:
    explicit TimerCallback(const std::string& name) : tracked_(name)
    {
    }
...
{
public:
    explicit DestructionTracker(const std::string& name) : tracked_(name), thread_id_(std::this_thread::get_id())
    {
...
{
public:
    explicit WorkItem(int id) : tracked_("Work" + std::to_string(id)), id_(id)
    {
    }
...
{
public:
    explicit RaceConditionDemo(const std::string& name) : tracked_(name), counter_(0)
    {
    }
...
private:
    ThreadSafeSingleton() : tracked_("Singleton")
    {
    }
...
{
public:
    explicit ThreadedNode(const std::string& name) : tracked_(name)
    {
    }
...
TEST_F(MultiThreadedPatternsTest, CircularReferenceAcrossThreads)
{
...

    {
        ...
        std::thread t1([node1, node2]() { node1->set_next_unsafe(node2); });

        std::thread t2([node1, node2]() { node2->set_next_unsafe(node1); });
...
{
    {
        ...
        std::thread t1([node1, node2]() { node1->set_next_safe(node2); });

        std::thread t2([node1, node2]() { node2->set_next_safe(node1); });
...
public:
    explicit CancellableTimer(asio::io_context& io, const std::string& name)
        : io_(io), timer_(io), tracked_(name), cancelled_(false)
    {
    }
...
public:
    explicit Connection(asio::io_context& io, int id)
        : io_(io), tracked_("Connection" + std::to_string(id)), id_(id), active_(true)
    {
...
{
public:
    explicit ConnectionManager(asio::io_context& io) : io_(io)
    {
    }
...
{
public:
    explicit BindVsLambda(const std::string& name) : tracked_(name)
    {
    }
    ...
    {
        // Using lambda
        asio::post(io, [self = shared_from_this()]() { self->method_with_args(42, "lambda"); });
